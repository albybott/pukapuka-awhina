/**
 * Waiata Himene pukapuka items
 */

module.exports = {
  items: [
    /* Item Start */
    {
      heading: { title: 'He Honore' },
      group: 'Waiata Himene',
      youtube: 'https://www.youtube.com/watch?v=EIw73tvLrUI',
      lines: [
        { maori: 'He Honore' },
        { maori: 'He korōria' },
        { maori: 'Maunga rongo ki te whenua' },
        { maori: 'Whakaaro pai e' },
        { maori: 'Ki ngā tāngata katoa' },
        { maori: 'Ake ake, ake ake, āmene' },
        { maori: 'Te Atua, te piringa, tōku oranga' },
      ],
    },
    /* Item Start */
    {
      heading: { title: 'Whakaaria Mai' },
      group: 'Waiata Himene',
      youtube: 'https://www.youtube.com/watch?v=9hxubCgwT-c&t=4s',
      lines: [
        { maori: 'Tō ripeka ki au' },
        { maori: 'Tiaho mai' },
        { maori: 'Rā roto i te po' },
        { maori: 'Hei kōnā au' },
        { maori: 'Titiro atu ai' },
        { maori: 'Hei au koe noho ai' },
      ],
    },
    /* Item Start */
    {
      heading: { title: 'Tama Ngākau', friendly: 'Tama Ngakau' },
      group: 'Waiata Himene',
      youtube: 'https://www.youtube.com/watch?v=zvh-_h0M0Yk&list=RDzvh-_h0M0Yk',
      lines: [
        { maori: 'Tama a te Atua' },
        { maori: 'Tēnei tonu mātou' },
        { maori: 'Arohaina mai' },
        { maori: '' },
        { maori: 'Murua rā ngā hara' },
        { maori: 'Wetekina mai' },
        { maori: 'Ēnei here kino' },
        { maori: 'Whakararu nei' },
        { maori: '' },
        { maori: 'Tēnei arahina' },
        { maori: 'A tutuki noa' },
        { maori: 'Puta i te pouri' },
        { maori: 'Whiwhi hari nui	' },
      ],
    },
    /* Item Start */
    {
      heading: { title: 'Ka waiata ki a Maria' },
      group: 'Waiata Himene',
      youtube: 'https://www.youtube.com/watch?v=1kcUul5LKn0',
      lines: [
        { maori: 'Hine i whakaae' },
        { maori: 'Whakameatia mai' },
        { maori: 'Te whare tangata' },
        { maori: '' },
        { maori: 'Hine pūrotu' },
        { maori: 'Hine ngākau' },
        { maori: 'Hine rangimarie' },
        { maori: 'Ko te whaea' },
        { maori: 'Ko te whaea o te ao' },
      ],
    },
    /* Item Start */
    {
      heading: { title: 'Piki mai, kake mai', friendly: 'Piki mai kake mai' },
      group: 'Waiata Himene',
      youtube: 'https://www.youtube.com/watch?v=LLx7QVgP8W8',
      lines: [
        { maori: 'Whakarongo ki te tangi' },
        { maori: 'a te huia hui hui mai tatou' },
        { maori: 'Whitiwhiti kōrero, whuia ki te ao e' },
        { maori: 'Hei mātakitaki, hei whakarongorongo' },
        { maori: 'Mā te tini mē te mano e' },
      ],
    },
    /* Item Start */
  ],
}
